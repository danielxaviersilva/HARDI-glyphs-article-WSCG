...
For the sake of performance, we suggest that the mesh used is symmetrical and its points' form lower resolution spherical meshes.

The mesh is symmetrical if $P \in \Pi \implies -P \in \Pi$. We also suggest that the data structure sent to GPU has the pair of symmetrical points grouped, as $[P_1, -P_1, P_3, -P_3, \dots, P_{N-3}, -P_{N-3}, P_{N-1}, -P_{N-1}]$, where $P_{2K+2} = -P_{2K+1}$, $(0 \leq K < N/2)$.
...
These meshes can be alternated following Voltoline and Wu's trade-off $\tau > 8\sqrt{max_p}$; $max_p > 0$ \cite{voltoline2021}, where $\tau$ is the base mesh amount of triangles and $max_p$ the maximum amount of pixels containing a single DWI's voxel. This trade off establishes the minimum amount of triangles that does not sacrifice the image quality.

Let us define a set $\Delta$ of $t$ elements, where each element $\delta_k = (\tau_k/8)^2$; ($0 < k \leq t$). Keeping the minimum amount of triangles, given a $max_p$, we suggest a heuristic that chooses one of the $t+1$ meshes that has the lowest amount of triangles that satisfies the trade off:
...
For the $(\Pi_k, I_k)$ mesh chosen, one must be attentive to two procedures to select the mesh at run time. The first consists in the activation of the index set $I_k$ and, since the amount of the mesh vertices $N_k$ is smaller than $N$, the ODF data per glyph is reduced by a factor of $1-V_k/V_n$, as discussed further, in section !!!!!!!!!.
...
The strategies exposed aims to decrease the amount of data per glyph to only three floats that refers to the translation coordinates; and $N_k/2$ floats that refer to ODF samples, where $N_k$ is the number of vertices of the chosen mesh.
...
First, we propose a formulation of discrete spherical mesh sets that present suitable properties to be a base mesh for precomputed ODF samples and can be changed from one to another at run time, then we present a criterion that defines the mesh to be used to render the ODF as a function of its pixel occupancy estimation, where we present the Voltoline and Wu's \cite{voltoline2021} metrics to adjust it, then we describe the procedures that one must be attentive to alternate between meshes; in the appendix we provide an application of the presented concepts to the spherical mesh generated by the $2^k$-th order of tessellation of the icosahedron.
...
% In their work, they propose an expression that establishes a relation between the mesh's amount of triangles $\tau$ and the , which is given by:

%\begin{equation}
%\label{eq::pixel_res}
 %   \tau (max_p) = \lfloor 2(3 + 2\sqrt[4]{max_p})^2 \rfloor
%\end{equation}$1-V_k/V_n$.
...
An ODF is symmetrical if $R(\bm{u}) = R(-\bm{u})$ for all $\bm{u}$ in the unit sphere. This symmetry applies to the diffusion behavior, which makes this section entirely applicable for rendering HARDI ODFs.%This property is present on all imaging methods for diffusion.
...
Each mesh glyph $i$ is individualized by two sets of attributes of the corresponding voxel: \textcolor{blue}{min-max normalized} ODF samples ($R_i(\bm{u})$) and displacement $(dx, dy, dz)$ relative to the origin.
...
The ODF data sent to GPU consists of the N/2 samples referring to the min-max normalized diffusion profiles for each $M$ glyphs to be drawn. We construct a $\bm{\Psi}_{NxM}$ matrix, where $M$ is the number of glyphs to be drawn, and $N$ is the number of samples of ODF used, as given in Eq. \ref{eq::Psi}:
...
\setlength\arraycolsep{2pt}
\begin{bmatrix} 
    R_1(\bm{u}_1) & R_1(\bm{u}_2) & \cdots R_1(\bm{u}_{M-1}) & R_1(\bm{u}_{M})  \\    
     R_2(\bm{u}_1)& R_2(\bm{u}_2) & \cdots R_2(\bm{u}_{M-1}) & R_2(\bm{u}_{M}) \\
    \vdots & \vdots & \vdots & \vdots  \\    
     R_N(\bm{u}_1) & R_N(\bm{u}_2) & \cdots R_N(\bm{u}_{M-1}) & R_N(\bm{u}_{M})
\end{bmatrix}
\endgroup
\end{equation}

%$r_{ij}$ refers to the point in the i-th ODF surface, which is obtaining by scaling the vertex $P_j$ of the spherical mesh. $\bm{R}_{NxM}$ is sent to the GPU as a 2D texture with RGBA format, which is used to maximize the occupancy in GPU's memory. Thus, the pair width and height are given by $(N/4, M)$. If $N$ is not divisible by four, one has to fulfill the remaining values with dummy values so it becomes divisible by for, it is necessary so that the first samples of other ODFs do not occupy the RGBA values of the previous.

The per-glyph ODF samples are accessed on GPU by its Vertex\_ID. As the elements of the same ODF are disposed in sequence in memory, the RGBA texture sample gives four values of ODF, which are associated to eight points of the mesh. The Vertex\_ID 8K and 8K+1 are associated with the R value of the sample; the pair 8K+2 and 8K+3 are associated with the G sample; the pair 8K+4 and 8K+5 are associated with the B sample and 8K+6 and 8K+7 refers to the A sample. Furthermore, K refers to the index that, where K = 0 establish the samples to 8 first ODFs, K = 1, to the next eight and so forth.

The rows refer to a set of samples of one ODF, which can be accessed directly by the Instance\_ID.

Finally, the OpenGL command to perform ODF lookup to deform the spherical mesh for a vertex being processed is texelFetch($\bm{R}$,ivec2((gl\_VertexID)/8, gl\_InstanceID), 0)[(gl\_VertexID\%8)/2].
...
\subsection{Multimodal rendering application}
...
%\end{algorithm}
\section{Appendix}
\subsection{Application with two-powered set of tessellated icosahedron meshes}
