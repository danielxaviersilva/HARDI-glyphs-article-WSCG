...
Diffusion magnetic resonance imaging provides the quantification of the diffusion process of water molecules. Applied to the brain, it is unique on providing white matter fiber configuration in-vivo. In order to synthesize in detail the diffusion displacement, high angular resolution imaging (HARDI) methods are used and these techniques are represented by orientation distribution functions (ODF), which consist of an association between directions to a probability of occurrence set. In this work we present an interactive polygon-based rendering scheme for ODFs. In our approach, the ODFs are represented by samples in a spherical domain and we achieve interactive rates to render multiple glyphs of them by minimizing the CPU-GPU data traffic and using the instance rendering technique. The interactivity of the scheme may help researchers in the area to improve their understanding on local diffusion displacement profiles.
