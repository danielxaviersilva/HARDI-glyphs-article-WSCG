...
\begin{figure}[h]
    ...
    \caption{Glyphs integrated into a DWI's visualization scheme. The glyphs are located in a midaxial region of the brain, cutted in the left side %. The spherical mesh used corresponds to an 8-th tessellated icosahedron
    }
...
To make this possible, there is a functionality to detect the appearing samples on screen, returning their respective indexes of the volume, feature proposed by Voltoline et al. \cite{voltoline2021}. The translation coordinates buffer and ODF data are created by copying the precomputed data in the fashion discussed in the subsection \ref{ssec::datastruct} and sent to GPU, as illustrated in Fig. \ref{fig::vmtk_precomputed2GPU}.
...
The rendering scheme is not only DWI applicable, but to other areas that use spherical functions, represented by samples.
