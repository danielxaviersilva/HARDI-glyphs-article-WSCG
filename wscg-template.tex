...
Our goal is to show the rendering scheme of the glyphs to be shown in the scene. The subsection \ref{ssec::precomputation} describes the procedures that must be done a priori; the subsection \ref{ssec::datastruct}, we describe the input data structures that customize the glyphs and are the most part of the CPU-GPU data traffic; in the subsection \ref{ssec::rendering}, we provide a summary of the CPU and GPU division of tasks and in subsection \ref{ssec::optimization}, we discuss an optimization that can be done in symmetrical ODFs.
...
The scheme's precomputation consists of setting a unit spherical mesh $ \{\bm{p}\} = \{P_1, P_2, \dots, P_N\}$ of $N$ points centered in the origin and its respective index buffer and send to the GPU only once as an attribute to be instanced.
...
Two elements customize each one of the glyphs: their position and ODF samples. These elements are sent to the GPU in every draw request.

The translation matrix places the center of the spherical mesh to its corresponding position. The ODF samples deform its respective spherical mesh. Let $P$ be a point of the spherical mesh, then lets define $\{\bm{U}\} = \{\bm{u}_k | \bm{u}_k = P_k - C, \forall P_k \in \{\bm{p}\} \text{ and $C$ is the center of the sphere}\}$. The glyph geometry is reshaped by scaling each point $P$ in the mesh to its respective ODF value $\psi(\bm{u})$ before translating it.
...
Let P be the amount of instances of the rendering scheme and the number of glyphs to be rendered. The translation matrices are organized as a vector with P elements and sent to the GPU as an attribute. This attribute is unique for each one of the P instances of the spherical mesh.

The i-th index of the matrices set vector refers to the spherical mesh's i-th instance to be rendered centered at the position given by the coordinates stored at this index.
...
The instance index (Instance\_ID) corresponds to an instanced spherical mesh to be rendered at a certain position. Since the ODFs samples of one glyph is stored in the rows of $\bm{\Psi}$, the Instance\_ID access this dimension.


The vertex index (Vertex\_ID), in its turn, indicates the points of the spherical mesh, which corresponds to the column of its respective ODF value in $\bm{\Psi}$.
...
\begin{figure}[htb]
    ...
    \caption{
    Illustration of $\bm{\Psi}$ in the GPU and how it is accessed for each vertex and instance. The vertices of the spherical mesh are in green. At each instance, the glyph's customization occurs through the scaling of the mesh point with its respective ODF value, recovered in the vertex shader through its vertex index for the P glyphs to be rendered.
...
In this subsection, we show an adaption of the data structure described in the subsection \ref{ssec::datastruct} to symmetrical ODFs and show an estrategy to decrease its related CPU-GPU data traffic by half.
...
Firstly, we suggest the use of a symmetrical spherical mesh. That means, if a point $P \in {\bm{p}} \implies -P \in {\bm{p}}$ as well. As an example, any order of a tessellated icosahedron fits this criterion.

Secondly, we recommend that the chosen symmetrical spherical mesh's data structure is organized so that a point $P$ in an even index is followed by its symmetrical point $-P$. This spherical mesh is sent to the GPU in the same form as stated in the subsection \ref{ssec::precomputation}.
...
Following these recommendations, the data structure of points in the sphere $[P_1, P_2, P_3, P_4, \dots, P_{N-1}, P_N]$ becomes  $[P_1, -P_1, P_3, -P_3, \dots, P_{N-1}, -P_{N-1}]$ and their associated $\{\bm{U}\}$ turns to  $[\bm{u}_1, -\bm{u}_1, \bm{u}_3, -\bm{u}_3, \dots, \bm{u}_{N-1}, -\bm{u}_{N-1}]$. In this situation, $\bm{\Psi}$ is now:
...
where, for symmetrical ODFs, each of the (2k+1)-th column is the same as the (2k+2)-th, for $k$ positive integer and smaller than $\frac{N}{2}$.

Hence, we define a matrix $\bm{\Psi^h}_{Px\frac{N}{2}}$ such that $\bm{\psi^h}_{ij} = \bm{\psi}_{i(2j-1)}$, which is described in the equation below:
