...
First, we propose a formulation of a discrete spherical mesh sets that present suitable properties to be a base mesh for precomputed ODF samples and can be changed from one to another at runtime, then we present a criterion that defines the mesh to be used to render the ODF as a function of its pixel occupancy estimation, where we present the Voltoline and Wu's \cite{voltoline2021} metrics to adjust it, then we describe the procedures that one must be attentive to alternate between meshes; in the subsection \ref{ssec::ico_example} we provide an application of the presented concepts to the spherical mesh generated by the $2^k$-th order of tessellation of the icosahedron.
...
\begin{enumerate}
    \item if $max_p < max_{p1}$, choose $(\Pi_1, I_1)$;
    \item if $max_{p1} < max_p < max_{pt}$ choose $(\Pi_k, I_k)$ such that $max_{p(k-1)} < max_p < max_{pk}$;
...
In this environment, a high order method is computed and stored in the CPU through its samples on a hemisphere (subsection \ref{sssec::optimization}) of a sphere obtained by the 16-th tessellation order of the icosahedron for all samples contained in a DWI, giving an total of 1281 samples per voxel. Their respective translation coordinates to its center in the volume space were computed as well.

The spherical mesh used is a function of $max_p$, where we alternated the $2^{nd}$, $4^{th}$, $8^{th}$ and $16^{th}$ order mesh as a function of $max_p$, whose values are in table \ref{tab::icosahedron_set}, the mesh were selected accordingly to the criteria described.
