...
The first and the most applied method in the clinical area is diffusion tensor imaging (DTI) \cite{Basser1994}. The method aims to fit a set of diffusion signal samples into a Gaussian 3D model.%, of average zero, and the diffusion tensor is the covariance matrix.
...
More advanced imaging methods for diffusion were introduced to overcome the limitation of the Gaussian model. These methods require more samples on the DWI acquisitions than those applied on DTI. These acquisitions are called High Angular Resolution Diffusion Imaging (HARDI). While the amount of diffusion-weighted acquisitions used on DTI vary between 6 and 32, HARDI acquisitions have more than 45 \cite{descoteaux2015}.
...
where $\bm{u}$ is a unity vector contained in the spherical domain, and $R(\bm{u})$ is the radius of the spherical mesh.

These glyphs provide a clear visualization of local diffusion through its ODF samples. Researchers use them to validate an imaging method \cite{descoteaux2007_QBI,  TuchQBall2004,tournier2007,Tournier2004DirectEO, tuch2002,  yeh2010}, assess the local relationship between the acquisition quality, the imaging method, and the underlying fiber orientation distribution detected (FOD) \cite{descoteaux2007}. The FODs extracted from an imaging method applied to a DWI acquisition are used as input of fiber tracking algorithms. The conjecture of the brain connectivity from the DWI acquisitions motivates this research area's main motivation. %Inferring brain tracts from DWI acquisitions consists on the main motivation of the research in the area.
...
In this work, we present an interactive GPU-accelerated rendering scheme of multiple spherical polar plots from ODF samples. We applied instance rendering to reduce the CPU-GPU data traffic.%\sout{The gains of performance are given by decreasing the amount of data traffic CPU-GPU by taking out redundant information on each drawing request. This is achieved by using instance rendering.}
...
This paper is divided in \ref{sec::conclusions} sections. In section \ref{sec::related_work}, we discuss the related works; in the section \ref{sec::odf_glyph_rendering} we discuss the rendering scheme, the data structures associated, an overview of the GPU shaders, and an optimization for symmetrical ODFs; in the section \ref{sec::results} we analyze the glyphs generated and show performance measurements. In the section \ref{sec::conclusions} we show the conclusions of the work.
...
%ALGO RELEVANTE: ESSE TRABALHO DO SHATTUCK É O ÚLTIMO DA ÁREA QUE TEM ALGUMA COISA A VER COM O MEU, QUE É RENDERIZAR GLIFOS HARDI EM TREAL.
...
The scheme's precomputation consists of setting a unit spherical mesh of N vertices centered in the origin and its respective index buffer and send to the GPU only once as an attribute to be instanced.
...
Two elements customize each one of the glyphs: their position and their ODF samples. These elements are sent to the GPU in every draw request.

The translation matrix places the center of the spherical mesh to its corresponding position. The ODF samples represent the diffusion profiles that are particular to each glyph. The glyph geometry is reshaped by multiplying the mesh normal direction $\bm{n}$ to its respective ODF value $\Psi(\bm{n})$.

Due to the large amount of data sent to the GPU in the drawing requests, the goal of this subsection is to show a way to organize the data in the CPU, how to send it to the GPU, and how it is read by the GPU shaders, so the CPU-GPU data traffic is as minimum as possible. %In this subsection, we are going to describe the general data structures of the data and in the subsection \ref{ssec::optimization}, we present an optimization of the scheme for ODFs that have symmetry.
...
Let P be the number of glyphs to be rendered on the screen. The translation matrices are organized as a vector of translation matrices with P elements and sent to the GPU as an attribute. The i-th index of the matrices set vector refers to the spherical mesh's i-th instance to be rendered centered at that position.
...
The ODF data sent to GPU consists of the N samples referring to the diffusion profiles for each one of the P glyphs to be drawn. We construct a $\bm{\Psi}_{PxN}$ matrix, where P is the number of glyphs to be drawn, and N is the number of samples of ODF used, as given in Eq. \ref{eq::Psi}.
...
where $\psi_{ij}$ refers to the ODF $\psi_i(\bm{u})$ of the i-th glyph, associated with the j-th vertex of the sphere, at which the sphere normal is in the direction $ \bm{u}_j$. The $\bm{\Psi}_{PxN}$ matrix is sent to the GPU as a 2D texture.

On the GPU, the access of ODFs to deform its respective sphere point is done by accessing the texture data through its instance and vertex indexes in the vertex shader. The vertex index (Vertex\_ID) of the direction of each vertex corresponds to the column of its respective ODF value $ \psi (\bm{u}_j) $ in diffusion profile matrix. The instance index (Instance\_ID), in its turn, corresponds to the row of diffusion profile at the respective vertex. Hence, we reduced GPU access performing a single lookup of the texel\footnote{In OpenGL, the lookup is done by the command texelFetch($\bm{\Psi}$, ivec(gl\_InstanceID, gl\_VertexID))} in the texture using the Vertex\_ID and Instance\_ID pair. Fig.\ref{fig::GPU2glyph} illustrates the access of the profile matrices on the GPU.
...
\begin{figure}[htb]
    ...
    \caption{
    Illustration of $\bm{\Psi}$ in the GPU and how it is accessed for each vertex and instance. The vertices of the spherical mesh are in green. At each instance, the glyph's customization occurs through the multiplication of the mesh vertex with its respective value, recovered in the vertex shader through its vertex index for the P glyphs to be rendered.
...
In this subsection, we show an adaption of the data structure described in the subsection \ref{ssec::datastruct} to symmetrical ODFs and decrease its related CPU-GPU data traffic by half.

An ODF is symmetrical if $\psi(\bm{u}) = \psi(-\bm{u})$ for all $\bm{u}$ in the unit sphere. This symmetry applies to the diffusion behavior, which makes this section totally applicable for rendering HARDI ODFs.  %This property is present on all imaging methods for diffusion.

Firstly, we suggest the use of a symmetrical spherical mesh. That means, if a point $P$ is contained in the spherical mesh vertices set, the point $-P$ is also contained. As an example, any order of a tessellated icosahedron fits this criterion.

Secondly, we recommend that the chosen symmetrical spherical mesh's data structure is organized so that a point $P$ in an even index is followed by its symmetrical point $-P$.
...
Following these recommendations, The data structure of points $[P_1, P_2, P_3, P_4, \dots, P_{N-1} P_N]$ becomes  $[P_1, -P_1, P_3, -P_3, \dots, P_{N-1}, -P_{N-1}]$ and their associated normals: $[\bm{u}_1, -\bm{u}_1, \bm{u}_3, -\bm{u}_3, \dots, \bm{u}_{N-1}, -\bm{u}_{N_1}$. In this situation, $\bm{\Psi}$ becomes
...
where, for symmetrical ODFs, each of the (2k+1)-th column is the same as the (2k+2)-th, for $k$ integer positive and smaller than $\frac{N}{2}$.

We define a matrix $\bm{\Psi^h}_{Px\frac{N}{2}}$ such that $\bm{\psi^h}_{ij} = \bm{\psi}_{i(2j-1)}$, which is described in the equation below:
...
\section{Results}
\label{sec::results}
Some of the results applied to a DW-MRI in the corpus callosum region are shown in Fig. \ref{fig::ex_glyph} for different tesselations of the icosahedron. Fig. \ref{fig::ex_glyph_FAMAP} shows an application of the proposed scheme integrated into a DWI visualization system.
