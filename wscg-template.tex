...
The proposed rendering scheme has been implemented in a multimodal visualization environment to DWI. In this environment, there is a raycasting based rendering for MRI/DWI volumes and a system for appearing samples detection, which is proposed and described by Voltoline et al. \cite{voltoline2021}.

In this environment, a HARDI method is computed through its samples in a hemisphere of a spherical mesh for all voxels contained in a DWI. The visualization system renders the DWI volume, with the option to corregister with an anatomical T1, detect the voxels appearing on screen and reference the pre computed HARDI data, to compute $\mathbf{Psi}^h$.

More details of the approach on detecting appearing samples can be found on Voltoline et al. work \cite{voltoline2021}.

The Fig. \ref{fig::ex_glyph_FAMAP}
