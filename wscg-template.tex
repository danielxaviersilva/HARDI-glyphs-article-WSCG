...
The contribution of this paper is the proposal of an triangle-based interactive rendering scheme for ODFs glyphs represented by samples applied to a multimodal visualization environment for DWI. glyphs\textcolor{red}{....}
...
Shattuck et al. \cite{shattuck2008} \textcolor{blue}{tessellated a polar representation of an ODF with triangles by subdividing its polar domain \textcolor{magenta}{and generated its shape in CPU as a function of an analytical function in the sphere}. Whenever the modeling and viewing parameters were changed, the glyphs' vertices were recomputed and resent to the GPU.} The performance reported was 10 fps for a brain slice using a spherical mesh of 225 vertices per glyph, corresponding to approximately 2 million triangles \todo{How many glyphs? \textcolor{green}{They don't mention}}. \textcolor{blue}{We exploit, in this work, the modern GPU features to improve rendering performance, GPU-memory usage, and \todo{\textcolor{green}{The Raphael's concept of determining the mesh as function of how many pixels is occupied by a voxel is not something I am applying. In this work, the spherical mesh is fixed. That being said, there is a opportunity to do it}} screen occupancy.}
...
Voltoline and Wu \cite{voltoline2021} proposed a real-time triangle-based rendering scheme for superquadric tensor glyphs \cite{Kindlmann2004}, which have much fewer parameters than ODF glyphs. \todo{Many aspects were exploited to have performance gain!}. They exploit several modern GPU features to improve rendering performance, GPU-memory usage, and screen occupancy. Section \ref{sec:superquadric_rendering} gives a brief overview of their proposal. \sout{These results influenced us to adapt their scheme into our proposed HARDI glyph rendering scheme}\textcolor{blue}{We showed in this work how to structure the ODF data in a compliant way with the superquadric tensor data so that the ODF data can take advantage of \sout{the same} \textcolor{magenta}{a similar} GPU processing flow}.
...
In this section, we discuss the rendering for DTI's superquadrics and the ideas we have brought to our work.

Voltoline and Wu's \cite{voltoline2021} work goal is to show a pipeline for triangle-based superquadric glyph rendering in a 3D raycasted DWI rendering scheme co-registered with an anatomical T1 MRI, where each shape is positioned accordingly in its respective voxel.

They explore many GPU's features to make their approach interactive in several passes. Firstly, the relevant data to the rendering is loaded in GPU, including DTI data. In order to make their approach interactive, they show strategies to keep the CPU-GPU data traffic minmum, how to minimize the occupancy of the glyph's data on GPU's memory and how to make the amount of triangles on each glyph's mesh adaptative as a function of its dimension.

They use instance rendering to minimize glyphs' data GPU occupancy. The same base mesh is used and each glyph particular shape customizes the mesh by their respective voxel's DTI-derived and translation parameters.

The mesh is adaptative and set in each drawing request in GPU's tessellation shader. It consists in an uniform discretization of the spherical coordinates $(\phi, \theta)$ in the interval $[0,2\pi], [0, \pi]$ respectively. The amount of subdivisions is a function of the largest amount of pixels occupied by a volume's voxel. The higher the amount of pixels by occupied by a voxel, the bigger is the shape of the superquadric, and the work proposes a GPU-based algorithm to increase the resolution of the superquadric's mesh, accordingly.



\subsection{Applicability on ODF glyph rendering}

Due to the ODF's high data dimensionality, it is impractical to keep its data on GPU for rendering. Thus, our strategy consists on pre-computing ODF samples, store them on CPU and make them accessible by their respective indices. Then, we use the appearing voxel detection, return to CPU its data, extract the voxels' respective index set, organize the data and send to GPU to render.

Our biggest concern has been the CPU-GPU data traffic, which consists mainly on ODF's samples. In this work, we show strategies to minimize it.

The CPU-GPU data traffic minimization strategies of the data can be divided in two classes: 
\begin{enumerate}
    \item glyph customization, which refers about how can one minimize the data, keeping the vertices and triangles of an glyph fixed;
    \item mesh data, which refers about how can one decrease the resolution of the mesh in situations where there are many small glyphs being rendered.
\end{enumerate}
...
In this section we show a strategy to organize ODF samples so the amount of triangle of the spherical mesh can be adaptative as a function of the amount of glyphs being rendered. 

Voltoline and Wu \cite{voltoline2021} address the importance of an adaptive mesh in order to keep the rendering speed and image quality compromise. They proposed a computation of the mesh as a function of the size of the glyphs to rendered in the scene.
...
As we proposed an rendering based in ODF samples stored in CPU, we propose a procedure to alternate at run time the instanced spherical mesh, where its vertices are a subset of the vertices of the highest resolution mesh used to sample the ODFs.
...
Let $\Pi = \{P_1, P_2, ..., P_{N}\}$ be the vertices of the highest resolution spherical mesh, associated with its index buffer set $I$. Let $\Pi_{1}$, $\Pi_{2}$, ..., $\Pi_{t}$ be a set of vertices that define a spherical mesh such that:
...
\begin{align}
 \label{eq::subset_condition2}
   ...
   % &\Pi_{2} = \{P_1, P_2,... P_{V_1}, P_{V_1+1}, ..., P_{V_2}\} = \Pi_1 \cup \{ P_{V_1+1}..., P_{V_2}\} \nonumber\\
    ...
    &\Pi_{3} = \Pi_3 \cup \{ P_{V_2+1}..., P_{V_3}\} \nonumber\\
    &... \\
...
where $t$ is the amount of subsets of the spherical mesh vertices $\Pi$. The amount of vertices on each mesh follows the property $V_n > V_t > V_{t-1} > ... > V_2 > V_1$. Each $\Pi_{1}, \Pi_{2}, \Pi_{3}, ..., \Pi_{t}$ is associated with its respective index buffer $I_{1}, I_{2}, I_{3}, ..., I_{t}$ to form the spherical mesh surface's triangles. $\Pi_k$ and $I_k$ ($k \leq t$) define the k-th subset of the highest resolution spherical mesh.

In order to provide the possibility of resolution adjustment on the glyphs mesh, we suggest an additional task at the initialization of the rendering scheme. In addition to uploading the mesh's vertices set of the highest resolution $\Pi$ as a data structure $[P_1, P_2, ..., P_N]$, alongside its respective index buffer, we suggest that the $t$ index buffers set that form the triangles using the points of $\Pi$'s subsets to be upload.
...
One must be attentive to two details in the implementation for the chosen k-th subset of the highest resolution, in order to alternate between meshes. The first consists in the activation of the spherical mesh by only binding the selected spherical mesh's index buffer $I_k$. The second consists on setting the amount of columns $V_k$ that build the ODF matrix sent to GPU, which consists of the $V_k$ first ODF samples and must be coherent to the mesh used, thus reducing the CPU-GPU data-traffic in a factor of $1-V_k/V_n$.
...
\subsection{Example of a $2^n$ set of tesselated icosahedron meshes}

In this section, we show an application on how to alternate meshes of spherical meshes on ODF's sampled over a two-powered order icosahedron's tesselation.

The tessellated icosahedron is a very used set of spherical meshes in the HARDI community. The reasons consists in a relative uniformity of its points over the sphere and its respective index buffer to form the triangles is computed analytically. Additionaly,  for $k < n$, the vertices of the $2^{k}$ order tesselated icosahedron is a subset of $2^{n}$!!(Verificar quem disse isso)

In this section, we are going to consider $\Pi$ as sphere generated by the $16^{th}$ tesselation order of the icosahedron, which gives $V_N = 2562$ points and is a fine mesh to generate an glyphs!!JUSTIFICAR ISSO AQUI MELHOR.  Thus, Let us assign $\Pi_1$, $\Pi_2$ and $\Pi_3$ to be the vertices set of the $2^{nd}$, $4^{th}$ and $8^{th}$ order of the tessellated icosahedron, respectively. They satisfy the properties described in Eq. \ref{eq::subset_condition1} and \ref{eq::subset_condition2}. The table \ref{tab::icosahedron_set} shows the amount of vertices and triangles of each tessellation.
...
16             & 2562               & 7680                \\ \hline
\end{tabular}
\caption{Amount of vertices and triangles of 2-powered tessellations of the icosahedron. For $k<n$, the vertices on the tesselation $2^k$ is a subset of $2^n$.}
