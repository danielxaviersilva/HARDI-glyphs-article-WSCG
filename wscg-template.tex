...
One may compare our approach with Shattuck et al.'s \cite{shattuck2008}, since both of them are based on polygons, where ODF samples deform a base spherical mesh. In their approach, the ODF samples were stored as coefficients of basis functions on CPU, the glyphs were computed on slices and their approach consisted on the computation of each glyph's surface points displacing ODF samples on a base spherical mesh on CPU, and then use commands to delimit a set of primitives to draw.
