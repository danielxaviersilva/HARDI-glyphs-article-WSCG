...
Voltoline and Wu \cite{voltoline2021} show a pipeline for triangle-based superquadric glyph rendering in a 3D raycasted DWI rendering scheme co-registered with an anatomical T1 MRI, where each shape is positioned accordingly in its respective voxel.

They explore many GPU's features to make their approach interactive in several passes. Firstly, the DWI's $b0$, its respective anatomical T1 MRI, the matrix that co-register volumes and DTI data are upload. In order to make their approach interactive, they show strategies to keep the CPU-GPU data traffic minimum, how to minimize the occupancy of the glyph's data on GPU's memory and how to make the amount of triangles on each glyph's mesh adaptive as a function of its dimension.

Since their glyphs rendering is integrated with a MRI slice and a ray-casting volumetric rendering scheme, their strategies consists in: detect DWI's on-screen voxels volume coordinates; estimate the glyph's mesh resolution dynamically; render only the glyphs associated to on-screen voxels and minimize the occupancy of glyph's data on GPU's memory.

In order to detect on-screen voxels and estimate the mesh resolution, they propose an estimation of the volume's coordinates on the onscreen buffer and computation of the maximum amount of pixels that contains one single voxel. The mesh is set in each drawing request in GPU's tessellation shader.% It consists in an uniform discretization of the spherical coordinates $(\phi, \theta)$ in the interval $[0,2\pi], [0, \pi]$ respectively.

They use instance rendering to minimize glyphs' data GPU occupancy. The same base mesh is used for all glyphs and each voxel's DTI-derived parameters customize each shape into its respective superquadric, which is translated to the center of its respective voxel in the volume space.
...
Due to the ODF's high data dimensionality, it is impractical to keep its data on GPU for rendering. Thus, our strategy consists on pre-computing ODF samples, store them on CPU and make them accessible by their respective voxel's indices. Then, we use the appearing voxel detection, return to CPU its data, extract the voxels' respective index set, organize the data and send to GPU to render.
...
\begin{enumerate}
    ...
    \item mesh data, which refers about how can one change the resolution of the mesh accordingly.
\end{enumerate}

In our work, we show an rendering scheme that the input consists in a set of voxel indices, whose glyphs are to be rendered and its data, which consists of its translation matrix to the volume space and ODF data is pre-computed.

In a application that integrates DWI and anatomical visualization, we advise the reader to use this work integrated with the on-screen volume's voxel detecting scheme proposed by Voltoline and Wu \cite{voltoline2021}.
...
The sphere mesh obtained by two-powered tessellation presents the following property:  if $k < n$, the vertices of the $2^{k}$ order tessellated icosahedron is a subset of $2^{n}$. The tessellation of order $2^k$ consists in a subdivision of each triangle of the the $2^{k-1}$ tessellation in four. The vertices that makes the subdivision are the ones on each triangle are the ones present in the $2^{k-1}$ tessellation order in addition to their respective median points of each triangle's side. The table \ref{tab::icosahedron_set} shows the amount of vertices and triangles of each tessellation.

Thus, assigning $\Pi$ to be the vertices set of the $2^k$-th order of tessellation of the icosahedron and $\Pi_1$, $\Pi_2$, $\Pi_{k-1}$ to be the $2^{1}$-th, $2^{2}$-th, ..., $2^{k-1}$-th order, respectively, we obtain a set of meshes that satisfy the properties described in the expressions \ref{eq::subset_condition1} and \ref{eq::subset_condition2}. In practice, we recommend $k$ to be equal to 3 or 4, which corresponds to 642 and 2562 samples on the sphere. Values of k above that may occur in a prohibitive amount of memory for ODF samples for a DWI.
...
8              & 642                & 1920                \\ \hline
16             & 2562               & 7680                \\ \hline
$2^k$          & $10(2^k+1)(2^k-1)+12$ & $30.2^k$           \\ \hline
\end{tabular}
